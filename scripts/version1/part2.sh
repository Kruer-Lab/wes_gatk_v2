...
perl ${ANNOVAR_DIR}/table_annovar.pl  ${TRIO}.snp.hardfilt.vcf  ${ANNOVAR_DIR}/humandb/  -buildver hg38 -out ${TRIO}.snp.hardfilt -remove -protocol refGene,cytoBand,esp6500siv2_all,ALL.sites.2015_08,ljb26_all,exac03,dbnsfp42c,revel,intervar_20180118,clinvar_20220320,gnomad30_genome -operation g,r,f,f,f,f,f,f,f,f,f -nastring . -vcfinput
...
less ${TRIO}.snp.hardfilt.hg38_multianno.vcf | perl -lane 'print if /#/; $F[7] =~ /ExAC_ALL=(\d*).(\d*);/; $maf_exac=$1.".".$2; next if int($maf_exac) > $MAF_DEN; $F[7] =~ /ALL.sites.2015_08=(\d*).(\d*);/; $maf_1000g=$1.".".$2; next if int($maf_1000g) > $MAF_DEN; $F[7] =~ /esp6500siv2_all=(\d*).(\d*);/; $maf_esp=$1.".".$2; next if int($maf_esp) > $MAF_DEN; print' > ${TRIO}.snp.hardfilt.rare.vcf


echo -e "\n\n*********************** Denovo SNP: Select coding variants *********************** \n\n"
less ${TRIO}.snp.hardfilt.rare.vcf | egrep -v 'Func.refGene=intronic|Func.refGene=UTR3|Func.refGene=UTR5|Func.refGene=intergenic|Func.refGene=ncRNA|Func.refGene=downstream|Func.refGene=upstream' > ${TRIO}.snp.hardfilt.rare.coding.vcf

echo -e "\n\n*********************** Denovo SNP: Insersect trio denovo output with annotated rare variants  *********************** \n\n"
bedtools intersect -a ${TRIO}.snp.hardfilt.rare.coding.vcf -b ${TRIO}.snp.denovo.vcf -header > ${TRIO}.snp.denovo.intersect.vcf 
...
less ${TRIO}.indel.hardfilt.hg38_multianno.vcf | perl -lane 'print if /#/; $F[7] =~ /ExAC_ALL=(\d*).(\d*);/; $maf_exac=$1.".".$2; next if int($maf_exac) > $MAF_DEN; $F[7] =~ /ALL.sites.2015_08=(\d*).(\d*);/; $maf_1000g=$1.".".$2; next if int($maf_1000g) > $MAF_DEN; $F[7] =~ /esp6500siv2_all=(\d*).(\d*);/; $maf_esp=$1.".".$2; next if int($maf_esp) > $MAF_DEN; print' > ${TRIO}.indel.hardfilt.rare.vcf

echo -e "\n\n*********************** Denovo INDELs: Select coding variants *********************** \n\n"
less ${TRIO}.indel.hardfilt.rare.vcf | egrep -v 'Func.refGene=intronic|Func.refGene=UTR3|Func.refGene=UTR5|Func.refGene=intergenic|Func.refGene=ncRNA|Func.refGene=downstream|Func.refGene=upstream' > ${TRIO}.indel.hardfilt.rare.coding.vcf

echo -e "\n\n*********************** Denovo INDELs: Insersect trio denovo output with annotated rare variants  *********************** \n\n"
bedtools intersect -a ${TRIO}.indel.hardfilt.rare.coding.vcf -b ${TRIO}.indel.denovo.vcf -header > ${TRIO}.indel.denovo.intersect.vcf 
...
perl ${ANNOVAR_DIR}/table_annovar.pl  ${TRIO}.hom.filt.vcf  ${ANNOVAR_DIR}/humandb/  -buildver hg38 -out ${TRIO}.hom -remove -protocol refGene,cytoBand,esp6500siv2_all,ALL.sites.2015_08,ljb26_all,exac03,dbnsfp42c,revel,intervar_20180118,clinvar_20220320,gnomad30_genome -operation g,r,f,f,f,f,f,f,f,f,f -nastring . -vcfinput
...
########$GATK  VariantFiltration -R $REF -V ${TRIO}.hom.hg38_multianno.vcf  -filter "vc.getAttribute('CADD_phred').equals('.')" --filter-name "CADD" -filter "vc.getAttribute('CADD_phred') < $CADD_THR" --filter-name "CADD" -O  ${TRIO}.hom.cadd.varfilt.vcf
$GATK  VariantFiltration -R $REF -V ${TRIO}.hom.hg38_multianno.vcf  -filter "vc.getAttributeAsDouble('CADD_phred', 0) > $CADD_THR" --filter-name "High_CADD" -O  ${TRIO}.hom.cadd.varfilt.vcf
...
$GATK  VariantFiltration -R $REF -V ${TRIO}.hom.cadd.metasvm.vcf  -filter "(vc.getAttributeAsString('ExAC_ALL', null).equals('.') || vc.getAttributeAsDouble('ExAC_ALL', 0) < $MAF_REC) && (vc.getAttributeAsString('ALL.sites.2015_08', null).equals('.') || vc.getAttributeAsDouble('ALL.sites.2015_08', 0) < $MAF_REC) && (vc.getAttributeAsString('esp6500siv2_all', null).equals('.') || vc.getAttributeAsDouble('esp6500siv2_all', 0) < $MAF_REC)" --filter-name "Rare" -O  ${TRIO}.hom.cadd.metasvm.varfilt.vcf
$GATK SelectVariants -R $REF -V ${TRIO}.hom.cadd.metasvm.varfilt.vcf -select 'FILTER =~ Rare' -O ${TRIO}.hom.cadd.metasvm.rare.0.vcf
uniq -f 7 ${TRIO}.hom.cadd.metasvm.rare.0.vcf > ${TRIO}.hom.cadd.metasvm.rare.vcf
...
perl ${ANNOVAR_DIR}/table_annovar.pl  ${TRIO}.merged.filt.vcf  ${ANNOVAR_DIR}/humandb/  -buildver hg38 -out ${TRIO}.merged -remove -protocol refGene,cytoBand,esp6500siv2_all,ALL.sites.2015_08,ljb26_all,exac03,dbnsfp42c,revel,intervar_20180118,clinvar_20220320,gnomad30_genome -operation g,r,f,f,f,f,f,f,f,f,f -nastring . -vcfinput
...
$GATK  VariantFiltration -R $REF -V ${TRIO}.comphet.recode.vcf  -filter "vc.getAttributeAsDouble('CADD_phred', 0) > $CADD_THR " --filter-name "High_CADD" -O  ${TRIO}.comphet.cadd.varfilt.vcf
...
echo -e "\n\n *********************** Remove duplicate variants and  ***********************\n"
$GATK  VariantFiltration -R $REF -V ${TRIO}.comphet.cadd.metasvm.vcf  -filter "(vc.getAttributeAsString('ExAC_ALL', null).equals('.') || vc.getAttributeAsDouble('ExAC_ALL', 0) < $MAF_REC) && (vc.getAttributeAsString('ALL.sites.2015_08', null).equals('.') || vc.getAttributeAsDouble('ALL.sites.2015_08', 0) < $MAF_REC) && (vc.getAttributeAsString('esp6500siv2_all', null).equals('.') || vc.getAttributeAsDouble('esp6500siv2_all', 0) < $MAF_REC)" --filter-name "Rare" -O  ${TRIO}.comphet.cadd.metasvm.varfilt.vcf
...
perl ${ANNOVAR_DIR}/table_annovar.pl  ${TRIO}.xlink.filt.vcf  ${ANNOVAR_DIR}/humandb/  -buildver hg38 -out ${TRIO}.xlink -remove -protocol refGene,cytoBand,esp6500siv2_all,ALL.sites.2015_08,ljb26_all,exac03,dbnsfp42c,revel,intervar_20180118,clinvar_20220320,gnomad30_genome -operation g,r,f,f,f,f,f,f,f,f,f -nastring . -vcfinput
...
######$GATK  VariantFiltration -R $REF -V ${TRIO}.xlink.hg38_multianno.vcf  -filter "vc.getAttribute('CADD_phred').equals('.')" --filter-name "CADD" -filter "vc.getAttribute('CADD_phred') < $CADD_THR" --filter-name "CADD" -O  ${TRIO}.xlink.cadd.hardfilt.varfilt.vcf
$GATK  VariantFiltration -R $REF -V ${TRIO}.xlink.hg38_multianno.vcf  -filter "vc.getAttributeAsDouble('CADD_phred', 0) > $CADD_THR " --filter-name "High_CADD" -O  ${TRIO}.xlink.cadd.varfilt.vcf
...
echo -e "\n\n *********************** Remove duplicate variants and  ***********************\n"
$GATK  VariantFiltration -R $REF -V ${TRIO}.xlink.cadd.metasvm.vcf  -filter "(vc.getAttributeAsString('ExAC_ALL', null).equals('.') || vc.getAttributeAsDouble('ExAC_ALL', 0) < $MAF_REC) && (vc.getAttributeAsString('ALL.sites.2015_08', null).equals('.') || vc.getAttributeAsDouble('ALL.sites.2015_08', 0) < $MAF_REC) && (vc.getAttributeAsString('esp6500siv2_all', null).equals('.') || vc.getAttributeAsDouble('esp6500siv2_all', 0) < $MAF_REC)" --filter-name "Rare" -O  ${TRIO}.xlink.cadd.metasvm.varfilt.vcf
...
perl ${ANNOVAR_DIR}/table_annovar.pl ${TRIO}.dom.filt.vcf  ${ANNOVAR_DIR}/humandb/  -buildver hg38 -out ${TRIO}.dom -remove -protocol refGene,cytoBand,esp6500siv2_all,ALL.sites.2015_08,ljb26_all,exac03,dbnsfp42c,revel,intervar_20180118,clinvar_20220320,gnomad30_genome -operation g,r,f,f,f,f,f,f,f,f,f -nastring . -vcfinput
...
$GATK VariantFiltration -R $REF -V ${TRIO}.dom.hg38_multianno.vcf  -filter "vc.getAttributeAsDouble('CADD_phred', 0) > $CADD_THR " --filter-name "High_CADD" -O  ${TRIO}.dom.cadd.varfilt.vcf
...
echo -e "\n *********************** Filter x-linked variants to find rare variants ***********************\n\n"
echo -e "\n\n *********************** Remove duplicate variants and  ***********************\n"
$GATK  VariantFiltration -R $REF -V ${TRIO}.dom.cadd.metasvm.vcf  -filter "(vc.getAttributeAsString('ExAC_ALL', null).equals('.') || vc.getAttributeAsDouble('ExAC_ALL', 0) < $MAF_DOM) && (vc.getAttributeAsString('ALL.sites.2015_08', null).equals('.') || vc.getAttributeAsDouble('ALL.sites.2015_08', 0) < $MAF_DOM) && (vc.getAttributeAsString('esp6500siv2_all', null).equals('.') || vc.getAttributeAsDouble('esp6500siv2_all', 0) < $MAF_DOM)" --filter-name "Rare" -O  ${TRIO}.dom.cadd.metasvm.varfilt.vcf
...
$GATK VariantsToTable -V $SNP_IN --show-filtered -F CHROM -F POS -F Gene.refGene -F ID -F REF -F ALT -F QUAL -F Func.refGene -F ExonicFunc.refGene -F AAChange.refGene -F AC -F ExAC_ALL -F esp6500siv2_all -F ALL.sites.2015_08 -F RadialSVM_pred -F SIFT_pred -F Polyphen2_HDIV_pred -F Polyphen2_HVAR_pred -F MutationTaster_pred -F REVEL -F MetaLR_pred  -F CADD_phred -F fathmm-MKL_coding_pred -F PROVEAN_pred -F GERP++_RS_rankscore -F M-CAP_pred -F DANN_rankscore -F VEST3_score -F MutationAssessor_pred -F LRT_pred -F SiPhy_29way_logOdds_rankscore -F integrated_fitCons_rankscore -F AF -O $SNP_OUT

$GATK VariantsToTable -V $INDEL_IN --show-filtered -F CHROM -F POS -F Gene.refGene -F ID -F REF -F ALT -F QUAL -F Func.refGene -F ExonicFunc.refGene -F AAChange.refGene -F AC -F ExAC_ALL -F esp6500siv2_all -F ALL.sites.2015_08 -F RadialSVM_pred -F SIFT_pred  -F Polyphen2_HDIV_pred -F Polyphen2_HVAR_pred -F MutationTaster_pred -F REVEL -F MetaLR_pred  -F CADD_phred -F fathmm-MKL_coding_pred -F PROVEAN_pred -F GERP++_RS_rankscore -F M-CAP_pred -F DANN_rankscore -F VEST3_score -F MutationAssessor_pred -F LRT_pred -F SiPhy_29way_logOdds_rankscore -F integrated_fitCons_rankscore -F AF -O $INDEL_OUT

$GATK VariantsToTable -V $HOM_IN --show-filtered -F CHROM -F POS -F Gene.refGene -F ID -F REF -F ALT -F QUAL -F Func.refGene -F ExonicFunc.refGene -F AAChange.refGene -F AC -F ExAC_ALL -F esp6500siv2_all -F ALL.sites.2015_08 -F RadialSVM_pred -F SIFT_pred  -F Polyphen2_HDIV_pred -F Polyphen2_HVAR_pred -F MutationTaster_pred -F REVEL -F MetaLR_pred  -F CADD_phred -F fathmm-MKL_coding_pred -F PROVEAN_pred -F GERP++_RS_rankscore -F M-CAP_pred -F DANN_rankscore -F VEST3_score -F MutationAssessor_pred -F LRT_pred -F SiPhy_29way_logOdds_rankscore -F integrated_fitCons_rankscore -F AF -O $HOM_OUT

$GATK VariantsToTable -V $COMPHET_IN  --show-filtered -F CHROM -F POS -F Gene.refGene -F ID -F REF -F ALT -F QUAL -F Func.refGene -F ExonicFunc.refGene -F AAChange.refGene -F AC -F ExAC_ALL -F esp6500siv2_all -F ALL.sites.2015_08 -F RadialSVM_pred -F SIFT_pred  -F Polyphen2_HDIV_pred -F Polyphen2_HVAR_pred -F MutationTaster_pred -F REVEL -F MetaLR_pred  -F CADD_phred -F fathmm-MKL_coding_pred -F PROVEAN_pred -F GERP++_RS_rankscore -F M-CAP_pred -F DANN_rankscore -F VEST3_score -F MutationAssessor_pred -F LRT_pred -F SiPhy_29way_logOdds_rankscore -F integrated_fitCons_rankscore -F AF -O $COMPHET_OUT

$GATK VariantsToTable -V $XLINK_IN --show-filtered -F CHROM -F POS -F Gene.refGene -F ID -F REF -F ALT -F QUAL -F Func.refGene -F ExonicFunc.refGene -F AAChange.refGene -F AC -F ExAC_ALL -F esp6500siv2_all -F ALL.sites.2015_08 -F RadialSVM_pred -F SIFT_pred  -F Polyphen2_HDIV_pred -F Polyphen2_HVAR_pred -F MutationTaster_pred -F REVEL -F MetaLR_pred  -F CADD_phred -F fathmm-MKL_coding_pred -F PROVEAN_pred -F GERP++_RS_rankscore -F M-CAP_pred -F DANN_rankscore -F VEST3_score -F MutationAssessor_pred -F LRT_pred -F SiPhy_29way_logOdds_rankscore -F integrated_fitCons_rankscore -F AF -O $XLINK_OUT

$GATK VariantsToTable  -V $DOMINANT_IN --show-filtered -F CHROM -F POS -F Gene.refGene -F ID -F REF -F ALT -F QUAL -F Func.refGene -F ExonicFunc.refGene -F AAChange.refGene -F AC -F ExAC_ALL -F esp6500siv2_all -F ALL.sites.2015_08 -F RadialSVM_pred -F SIFT_pred  -F Polyphen2_HDIV_pred -F Polyphen2_HVAR_pred -F MutationTaster_pred -F REVEL -F MetaLR_pred  -F CADD_phred -F fathmm-MKL_coding_pred -F PROVEAN_pred -F GERP++_RS_rankscore -F M-CAP_pred -F DANN_rankscore -F VEST3_score -F MutationAssessor_pred -F LRT_pred -F SiPhy_29way_logOdds_rankscore -F integrated_fitCons_rankscore -F AF -O $DOMINANT_OUT
