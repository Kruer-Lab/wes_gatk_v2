process DENOVO_SNPS {
    ...
    less ${family}.snp.rare.vcf | { egrep -v 'Func.refGene=intronic|Func.refGene=UTR3|Func.refGene=UTR5|Func.refGene=intergenic|Func.refGene=ncRNA|Func.refGene=downstream|Func.refGene=upstream' || true; } > ${family}.snp.rare.coding.vcf
...
process DENOVO_INDELS {
    ...
    # Select coding variants
    less ${family}.indel.rare.vcf | { egrep -v 'Func.refGene=intronic|Func.refGene=UTR3|Func.refGene=UTR5|Func.refGene=intergenic|Func.refGene=ncRNA|Func.refGene=downstream|Func.refGene=upstream' || true; } > ${family}.indel.rare.coding.vcf
...
process COMPOUND_HETEROZYGOUS {
    ...
    more ${family}.comphet.recode.vcf | { egrep 'RadialSVM_pred=D|ExonicFunc.refGene=stop|ExonicFunc.refGene=start|frameshift|deletion|insertion|splicing|#' || true; } | { egrep -v 'Func.refGene=intronic|Func.refGene=UTR3|Func.refGene=UTR5|Func.refGene=intergenic|Func.refGene=ncRNA|Func.refGene=downstream|Func.refGene=upstream|ExonicFunc.refGene=synonymous' || true; } > ${family}.comphet.metasvm.lof.vcf
    ...
    cat ${family}.comphet.cadd.varfilt.select.vcf | { egrep -v 'Func.refGene=intronic|Func.refGene=UTR3|Func.refGene=UTR5|Func.refGene=intergenic|Func.refGene=ncRNA|Func.refGene=downstream|Func.refGene=upstream' || true; } > ${family}.comphet.cadd.coding.vcf
...
process VARIANTS_TO_TABLE {
    ...
    input:
        tuple val(family), path(pedigree), path(snp_in), path(indel_in), path(hom_recessive_in), path(comp_het_in), path(xlinked_in), path(dominant_in)
    ...
    awk '{print \$3}' ${family}.comphet.cadd.metasvm.rare.csv | sort | uniq -d | { grep -F -f - ${family}.comphet.cadd.metasvm.rare.csv || true; } > ${family}.comphet.cadd.metasvm.rare.dup.noheader.csv
